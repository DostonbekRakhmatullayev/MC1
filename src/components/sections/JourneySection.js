  ...
  return (
    <section id="journey" className="section-padding bg-gradient-to-br from-gray-50 to-white relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-5">
        <div className="absolute top-0 left-1/4 w-64 h-64 bg-meat-red rounded-full"></div>
        <div className="absolute bottom-0 right-1/4 w-48 h-48 bg-gold rounded-full"></div>
      </div>

      <div className="container mx-auto px-4 relative z-10">
        {/* Section Header */}
        <div className={`text-center mb-16 transform transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
          <div className="inline-flex items-center space-x-2 px-6 py-3 bg-blue-500 text-white rounded-full mb-6 font-semibold">
            <span>Bizning Yo'limiz</span>
          </div>
          <h2 className="text-4xl md:text-5xl font-bold text-meat-red mb-6 leading-tight">
            Muvaffaqiyat tarixi
          </h2>
        ...
        <div className="relative">
          {/* Vertical line */}
          <div className="absolute left-1/2 transform -translate-x-1/2 h-full w-1 bg-gradient-to-b from-orange-400 via-red-400 via-purple-400 to-pink-400 rounded-full"></div>
          ...
          <div className="space-y-16">
            {journeySteps.map((step, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`flex items-center justify-center relative transform transition-all duration-1000 delay-${index * 200} ${
                    isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
                  }`}
                >
                  {/* Timeline dot */}
                  <div className={`absolute left-1/2 transform -translate-x-1/2 w-16 h-16 ${step.bgColor} rounded-full flex items-center justify-center border-4 border-white shadow-lg z-10`}>
                    <IconComponent className={`w-8 h-8 ${step.color}`} />
                  </div>

                  {/* Content card */}
                  <div className={`w-full max-w-md ${isEven ? 'mr-auto pr-16' : 'ml-auto pl-16'}`}>
                    <div className="bg-white rounded-xl shadow-lg p-6 hover-lift">
                      <h3 className={`text-xl font-bold mb-2 ${isEven ? 'text-left' : 'text-right'}`}>
                        {step.year}
                      </h3>
                      <h4 className={`text-lg font-semibold text-gray-800 mb-3 ${isEven ? 'text-left' : 'text-right'}`}>
                        {step.title}
                      </h4>
                      <p className={`text-gray-600 leading-relaxed ${isEven ? 'text-left' : 'text-right'}`}>
                        {step.description}
                      </p>
                    </div>
                  </div>
        ...
        <div className={`text-center mt-16 transform transition-all duration-1000 delay-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
          <div className="bg-gradient-to-r from-meat-red to-gold text-white rounded-2xl p-8 max-w-4xl mx-auto">
            <h3 className="text-2xl font-bold mb-4">Kelajakda yanada ko'p yutuqlar!</h3>
